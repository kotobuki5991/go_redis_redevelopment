...
func writeResponse(conn net.Conn, redisRequest RedisRequest) {
	...
	switch cmd {
	case "echo":
		echoCmdHandler(conn, cmd, args[0])
	case "ping":
		pingCmdHandler(conn)
	case "set":
		setCmdHandler(conn, cmd, args[0])
	case "get":
		pingCmdHandler(conn)
	default:
...
func echoCmdHandler(conn net.Conn, cmd string, args string){
...
func setCmdHandler(conn net.Conn, cmd string, args string){
